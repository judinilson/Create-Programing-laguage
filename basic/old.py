...
class Position:
    ...
    def advance(self, current_char=None):  # to move on to next index and update line and colum num
        self.idx += 1
        self.col += 1
...
class Token:
    ...
    def matches(self, type_, value):  # to verify if token matches with given type
...
class Lexer:
    ...
    def make_identifier(self):  # method to detect if is variable keyword or identifier
        id_str = ''
        pos_start = self.pos.copy()
    ...
    def make_not_equals(self):
        ...
        if self.current_char == '=':  # if the current char after '!' is equal than advnc nd return token
            self.advance()
            return Token(TT_NE, pos_start=pos_start, pos_end=self.pos), None

        self.advance()  # if is not equal char than advance nd return exception
        return None, ExpectedCharError(pos_start, self.pos, "'=' (after '!')")

    def make_equals(self):
        tok_type = TT_EQ  # first take char is a equal =
        pos_start = self.pos.copy()  # set pos start
        self.advance()  # and advance to next char

        if self.current_char == '=':  # if the next char is equal to another =
            self.advance()
            tok_type = TT_EE  # than set token type to double equals

        return Token(tok_type, pos_start=pos_start, pos_end=self.pos)  # nd return token

    def make_less_than(self):
        token_type = TT_LT  # set token type to LESS THAN
        pos_start = self.pos.copy()
        self.advance()

        if self.current_char == '=':  # if next char is equal than
            self.advance()
            token_type = TT_LTEQ  # set token type to LESS THAN EQUAL type

        return Token(token_type, pos_start=pos_start, pos_end=self.pos)

    def make_greater_than(self):
        token_type = TT_GT  # set token type to GREATER THAN
        pos_start = self.pos.copy()
        self.advance()

        if self.current_char == '=':  # if next char is equal than
            self.advance()
            token_type = TT_GTEQ  # set token type to GREATER THAN EQUAL type
...
class ParseResult:
    ...
    def failure(self, error):
        # should override(when should overrid or not when hvent advanced since
...
class Parser:
    ...
    def atom(self):
        ...
        elif tok.type == TT_IDENTIFIER:  # verifying if is var_name(identifier)
            res.register_advancement()
            self.advance()
...
class Number:  # storing number class
    ...
    def added_to(self, other):
        # Return true if the object argument is an instance of the classinfo(mean its check if the value a num
...
# this class is to keep track all var names and their values
class SymbolTable:
    def __init__(self):
        self.symbols = {}  # symbol dictionary
        self.parent = None

    # get variable name
    def get(self, name):
        value = self.symbols.get(name,
                                 None)  # take variable name and if cannot find it in dictionary take none
        # if the value is none we go to the parent symbol table nd check each symb for the value
        if value is None and self.parent:
            # if those hve a parent
            return self.parent.get(name)
        return value

    def set(self, name, value):  # set name in symbol tble
...
class Interpreter:
    ...
    def visit_VarAccessNode(self, node, context):
        ...
        # find var position and set to the var access node position
...
def run(fn, text):
    ...
    # assign global symbol tbl to context.symbol_table given the proper access
